...
import {
  ...
  FiCalendar, FiSettings, FiGrid, FiSun, FiMoon
} from 'react-icons/fi';
...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-[1000] transition-all duration-300 ${scrolled ? 'bg-[rgba(10,10,26,0.9)] backdrop-blur-[10px] py-[15px] shadow-[0_2px_8px_rgba(0,0,0,0.3)]' : 'bg-transparent py-[20px]'}`}>
      <div className="w-full max-w-[1200px] mx-auto px-[20px] flex justify-between items-center relative">
        ...
        <Link to="/" className="text-[1.4rem] font-[800] flex items-center gap-[8px] text-white">
          <span className="text-[#6C63FF]">Tuition</span>
          <span className="text-white">Hub</span>
        </Link>

        {/* Desktop Nav Links */}
        <div className={`md:static md:flex md:flex-row md:items-center md:bg-transparent md:w-auto md:p-0 md:gap-[10px] fixed top-full left-0 w-full bg-[#111128] flex-col p-[20px] gap-[16px] transition-all ${menuOpen ? 'flex max-h-[500px]' : 'hidden max-h-0 md:max-h-none overflow-hidden md:overflow-visible'}`}>
          ...
          {/* Mobile auth buttons */}
          {!user && (
            <div className="md:hidden flex flex-col gap-[10px] mt-[10px] pt-[15px]">
        ...
        <div className="flex items-center gap-[12px]">
          ...
          <button
            onClick={toggleTheme}
            className="p-[8px] rounded-[8px] bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white transition-all hover:bg-[rgba(255,255,255,0.1)] dark:bg-[rgba(255,255,255,0.1)] dark:hover:bg-[rgba(255,255,255,0.15)]"
          ...
          {!user ? (
            ...
            <div className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute top-[calc(100%+10px)] right-0 w-[240px] bg-[rgba(17,17,40,0.95)] backdrop-blur-[20px] rounded-[16px] shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden z-[100] animate-[fadeInDown_0.3s_ease]">
                  ...
                  <div className="p-[8px] flex flex-col gap-[2px]">
                    ...
                    {user.role === 'tutor' && (
                      <>
                        <Link to="/tutor-profile" className="flex items-center gap-[12px] px-[16px] py-[10px] rounded-[8px] text-[#b0b0cc] font-[500] text-[0.9rem] transition-all hover:bg-[rgba(255,255,255,0.05)] hover:text-white" onClick={() => setDropdownOpen(false)}>
                          <FiSettings /> Edit Tutor Info
                        </Link>
                        <Link to="/tutor-services" className="flex items-center gap-[12px] px-[16px] py-[10px] rounded-[8px] text-[#b0b0cc] font-[500] text-[0.9rem] transition-all hover:bg-[rgba(255,255,255,0.05)] hover:text-white" onClick={() => setDropdownOpen(false)}>
                          <FiBookOpen /> My Services
                        </Link>
                      </>
                    )}
                  </div>
